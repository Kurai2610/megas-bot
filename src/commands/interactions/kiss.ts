...
export const data = new SlashCommandBuilder()
  ...
  .addUserOption((option) =>
    option
      .setName("user")
      .setDescription("El usuario a besar")
      .setRequired(true)
  );
...
async function getRandomGif(): Promise<string> {
  try {
    const response = await fetch(
      `https://api.giphy.com/v1/gifs/random?api_key=${GIPHY_API_KEY}&tag=anime+kiss&rating=g`
    );
...
export async function execute(interaction: CommandInteraction): Promise<void> {
  try {
    ...
    const embed = new EmbedBuilder()
      .setDescription(
        `👩‍❤️‍💋‍👨 **${interaction.user.globalName}** besó a **${user.globalName}**`
      )
      .setImage(gifUrl)
      .setColor(0xa67ba6)
      .setFooter({
